...
# 逐行处理文件， fun 是传入的处理函数
# 比较省内存，处理特大文件用
def processFile(filename, fun=None, stripLine=False):
...
# 按行读取并返回处理后的对象列表
# fun 是对行进行处理的函数
# skipNone 为 True 时， 不添加 None 到列表
def readAndProcess(filename, fun, skipNone=True):
    result = []
    file = open(filename, 'r')
...
# 追加文件内容
def appendContent(filename, content):
