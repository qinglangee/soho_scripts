...
; (function (root, factory) {
    ...
    function processCode(example, index, codeType) {
        ...
        let n = countLeadingSpaces(html);
        // html = html.replace(/^\s{${n}}/gm, '');

        // g 表示全局替换 m 表示多行匹配
        let regex = new RegExp(`^\\s{${n}}`, "gm");
        html = html.replace(regex, "");

        // 如果设置了 data-lang 属性， 就用这个做 codeType
        if(stage.attr('data-lang')) {
            codeType = stage.attr('data-lang');
        }
